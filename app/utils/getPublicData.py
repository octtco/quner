...
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'quner.settings')
django.setup()
from app.models import User, TravelInfo

monthList = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'Suptember', 'October',
             'November', 'December']
...
def get_travel_info_data(province=None):
    if province:
        travel_info = TravelInfo.objects.filter(province=province)
    else:
        travel_info = TravelInfo.objects.all()

    return list(map(map_fn, travel_info))
